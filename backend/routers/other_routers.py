...
def send_negotiation_message(data: NegotiationMessage, user=Depends(get_current_user)):
    ...
    round_count = messages_col.count_documents({
        "contract_id": data.contract_id,
        "type": "negotiation"
    })

    if round_count >= 5:
        chat_log = list(messages_col.find({
            "contract_id": data.contract_id,
            "type": "negotiation"
        }).sort("timestamp", 1))
        log = [
...
def topup_wallet(data: WalletTopup, user=Depends(require_employer)):
    # Dev mode — add directly, no payment gateway needed
...
def override_pfi(data: PFIOverride, user=Depends(require_admin)):
    ...
    users_col.update_one(
        {"_id": ObjectId(data.freelancer_id)},
        {"$set": {"pfi_score": data.new_score}}
    )
    pfi_col.insert_one({
...
def resolve_dispute(data: DisputeVerdict, user=Depends(require_admin)):
    ...
    contracts_col.update_one(
        {"_id": ObjectId(dispute["contract_id"])},
        {"$set": {"status": "resolved"}}
    )
